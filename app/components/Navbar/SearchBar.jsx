...
const SearchBar = ({ searchQuery, setSearchQuery, isExpanded, setIsExpanded }) => {
  ...
  useEffect(() => {
    if (!isExpanded) {
      ...
      const safeIndex = textIndex >= 0 && textIndex < placeholderTexts.length ? textIndex : 0
      ...
      menuData.forEach(category => {
        if (category.label.toLowerCase().includes(searchQuery.toLowerCase())) {
          suggestions.push({
            ...
            link: category.link,
            image: category.submenu?.[0]?.image || "/images/relot front page img 13-6/topbnr3.jpg"
          })
        }
        ...
        category.submenu?.forEach(sub => {
          if (sub.label.toLowerCase().includes(searchQuery.toLowerCase())) {
            suggestions.push({
              ...
              link: sub.link,
              image: sub.image || "/images/relot front page img 13-6/topbnr3.jpg"
            })
          }
  ...
  if (isExpanded) {
    return (
      <>
        ...
        <div 
          className={`fixed top-0 left-0 w-full z-50 shadow-2xl transition-all duration-600 ease-out transform ${
            isClosing 
        ...
        >
          <div className="container mx-auto px-4 py-8">
            ...
            <div className="hidden lg:grid lg:grid-cols-3 gap-8 max-h-96 overflow-y-auto">
              ...
              <div className="lg:col-span-2 space-y-6">
                {searchQuery.trim() ? (
                  ...
                  >
                    <h3 className="text-2xl font-bold mb-6 text-white drop-shadow-lg">Search Results</h3>
                    {filteredSuggestions.length > 0 ? (
                      <div className="space-y-4">
                        {filteredSuggestions.map((suggestion, index) => (
                          <div
                            key={index}
                            onClick={() => handleSearch(suggestion.label, suggestion.link)}
                            className={`flex items-center gap-6 p-5 rounded-xl cursor-pointer transition-all duration-400 group border shadow-xl transform ${
                              isClosing ? 'opacity-0 translate-x-8' : 'opacity-100 translate-x-0 hover:-translate-y-1 hover:scale-[1.02]'
                            }`}
                            style={{
                              background: 'rgba(255, 255, 255, 0.25)',
                              backdropFilter: 'blur(20px)',
                          ...
                          >
                            ...
                            <div className="flex-1">
                              ...
                              {suggestion.category && (
                                <p className="text-sm text-white/80 font-medium drop-shadow">{suggestion.category}</p>
                              )}
                      ...
                      <div className="text-center py-12 rounded-xl border shadow-lg" style={{
                        background: 'rgba(255, 255, 255, 0.15)',
                        backdropFilter: 'blur(20px)',
                        WebkitBackdropFilter: 'blur(20px)',
                        borderColor: 'rgba(255, 255, 255, 0.3)',
                        ...
                        <p className="text-xl font-bold text-white drop-shadow-lg">No results found for "{searchQuery}"</p>
                        <p className="text-sm mt-2 text-white/80 font-medium drop-shadow">Try different keywords or browse categories below</p>
                      </div>
                    )}
                  </div>
                ) : (
                  <>
                    ...
                    <div 
                      ...
                      style={{
                        background: 'linear-gradient(135deg, rgba(239, 68, 68, 0.25), rgba(249, 115, 22, 0.25), rgba(236, 72, 153, 0.25))',
                        backdropFilter: 'blur(25px)',
                        WebkitBackdropFilter: 'blur(25px)',
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        {trendingCategories.map((category, index) => (
                          <div
                            key={index}
                            onClick={() => handleSearch(category.label, category.link)}
                            className={`relative group cursor-pointer rounded-xl overflow-hidden transition-all duration-400 border shadow-xl transform ${
                              isClosing ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0 hover:-translate-y-2 hover:scale-105'
                            }`}
                            style={{
                              background: 'rgba(255, 255, 255, 0.2)',
                              backdropFilter: 'blur(20px)',
                          ...
                          >
                            <div className="relative h-28 p-5">
                              <div className="absolute top-3 right-3">
                                <span className={`text-xs px-3 py-1 rounded-full text-white font-bold shadow-lg ${
                                  category.tag === 'Trending' ? 'bg-red-500/90' :
                                  category.tag === 'Popular' ? 'bg-orange-500/90' :
                                  category.tag === 'New' ? 'bg-green-500/90' : 'bg-purple-500/90'
                                }`}>
                                  {category.tag}
                                </span>
                              </div>
                              ...
                              <p className="text-sm text-white/80 group-hover:text-red-100 transition-colors duration-300 font-medium drop-shadow">Explore now</p>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    >
                      <h3 className="text-2xl font-bold mb-6 text-white drop-shadow-lg">Popular Searches</h3>
                        ...
                        {popularSearches.map((search, index) => (
                          <button
                            ...
                            className={`px-6 py-3 rounded-full text-sm font-bold text-white/95 transition-all duration-300 border shadow-lg transform ${
                              isClosing ? 'opacity-0 scale-75' : 'opacity-100 scale-100 hover:-translate-y-1 hover:scale-105'
                            }`}
                            style={{
                              background: 'rgba(255, 255, 255, 0.2)',
                              backdropFilter: 'blur(20px)',
              ...
              >
                ...
                {/* Quick Categories */}
                <div>
                  <h3 className="text-xl font-bold mb-6 text-white drop-shadow-lg">Quick Categories</h3>
                  <div className="space-y-3">
            ...
            <div className="lg:hidden">
              {searchQuery.trim() && filteredSuggestions.length > 0 && (
                ...
                >
                  <h3 className="text-lg font-bold mb-4 text-white drop-shadow-lg">Search Results</h3>
                  <div className="space-y-3">
