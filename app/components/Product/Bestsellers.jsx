...
import { Star, Tag, TrendingUp, Flame } from 'lucide-react';
import { getBestseller } from '../../lib/products';

export default function BestSeller({ productId, category }) {
  ...
  useEffect(() => {
    const fetchRelatedProducts = async () => {
      try {
        setLoading(true);
        const response = await getBestseller(productId, category);
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        ...
        <div className="text-center mb-8 md:mb-12 lg:mb-16">
          ...
          <div className="flex items-center justify-center gap-2 md:gap-3 mb-4 md:mb-6">
            <div className="flex items-center gap-1.5 md:gap-2 bg-red-50 text-red-600 px-3 md:px-4 py-1.5 md:py-2 rounded-full border border-red-200">
              <Flame className="w-3 h-3 md:w-4 lg:w-5 md:h-4 lg:h-5" />
              <span className="font-semibold text-xs md:text-sm uppercase tracking-wide">Trending Now</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8 justify-items-center">
          {relatedProducts.map((product, index) => {
            ...
            return (
              <Link key={product._id} href={`/products/${product._id}`} className="w-full max-w-xs">
                <div 
                ...
                >
                  ...
                  <div className="absolute top-2 md:top-3 lg:top-4 left-2 md:left-3 lg:left-4 z-20">
                    <div className="bg-gradient-to-r from-yellow-400 to-orange-500 text-white text-xs font-bold px-2 md:px-3 py-1 md:py-1.5 rounded-full shadow-lg flex items-center gap-1">
                      ...
                      {product.discount > 0 ? (
                        <div className="space-y-1">
                          ...
                          <div className="text-xs md:text-sm font-medium text-green-600">
                            Save ₹{((product.price * product.discount) / 100).toLocaleString()}
                          </div>
                        </div>
                      ) : (
                        <span className={`font-bold text-base md:text-lg lg:text-xl transition-colors duration-300 ${
        ...
        <div className="text-center mt-8 md:mt-10 lg:mt-12">
          <Link
            ...
            className="inline-flex items-center gap-2 md:gap-3 bg-gradient-to-r from-red-600 to-red-700 text-white font-bold px-6 md:px-8 py-3 md:py-4 rounded-full hover:from-red-700 hover:to-red-800 transition-all duration-300 transform hover:scale-105 hover:shadow-lg group text-sm md:text-base"
          >
            <span>View All Bestsellers</span>
            <TrendingUp className="w-4 h-4 md:w-5 md:h-5 group-hover:translate-x-1 transition-transform duration-300" />
          </Link>
        </div>
      </div>
    </section>
